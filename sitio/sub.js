var REST_URL = "http://www.proyectos.arcelia.net/clma/src/public/";
var COOKIE_NAME = 'tk';
	                    ...
	                    if (celdas[i] == "matricula") {
	                        ...
	                        switch (celdas[i]) {
	                            ...
	                            case "detalle":
                                    ...
                                    btnCalif.classList.add('btn', 'btn-info', 'btn-sm');
                                    btnCalif.textContent = 'Calificaciones';
                                    btnCalif.href = "05CalificacionesAlumno.html" + "?acc=calif&matricula=" + alumno.clave_alu;
...
function listarCalificacionesAlumnos(idT){
    var califCeldas = ["Materia", "Clave Materia", "Calificacion", "Detalle"];
    if(checkCookie(COOKIE_NAME)){
        nRow = 1;
	    var tabla = document.getElementById(idT);
        ...
        var matricula = getQueryVariable("matricula");
	    var endPoint = REST_URL + "alumno/" + matricula + "/calificaciones";
	    ...
	    fetch(endPoint, requestOptions )
	        ...
	        .then(
	        	data => {
	            data.data.forEach(calif => {
	                var renglon = tabla.insertRow(nRow);
	                for (i = 0; i < califCeldas.length; i++) {
	                    var celda = renglon.insertCell(i);
	                    switch (califCeldas[i]) {
                            case "Materia":
                                var textoCelda = document.createTextNode(calif.nombreMateria);
                                break;
                            case "Clave Materia":
                                var textoCelda = document.createTextNode(calif.claveMateria);
                                break;
                            case "Calificacion":
                                var textoCelda = document.createTextNode(calif.calificacion);
                                break;
                            case "Detalle":
                                var textoCelda = document.createElement('div');
                                textoCelda.classList.add('bd-example');

                                var btnEdita = document.createElement('a');
                                btnEdita.classList.add('btn', 'btn-primary', 'btn-sm');
                                btnEdita.textContent = 'Edita';
                                btnEdita.onclick = function(){editaCalificacion(matricula, calif.claveMateria)};

                                var btnElimina = document.createElement('a');
                                btnElimina.classList.add('btn', 'btn-success', 'btn-sm');
                                btnElimina.textContent = 'Elimina';
                                btnElimina.onclick = function(){eliminaCalificacion(matricula, calif.claveMateria)};
                                
                                textoCelda.appendChild(btnEdita);
                                textoCelda.appendChild(btnElimina);
...
function editaCalificacion(matricula, clave_mat) {
    var nuevaCalificacion = prompt("Cual es la nueva calificacion?");
    ...
    urlencoded.append("calificacion", nuevaCalificacion);
    urlencoded.append("clave_mat", clave_mat);
...
function eliminaCalificacion(matricula, clave_mat) {
    var eliminar = confirm("Seguro que quiere eliminar?");
...
function agregaCalificacion() {
    var matricula = getQueryVariable("matricula");
    var materia = document.getElementById('clave_mat').value;
    ...
    var urlencoded = new URLSearchParams();
    urlencoded.append("token", token);
    urlencoded.append("id_curso", "C012");
    urlencoded.append("id_salon", "139");
    urlencoded.append("clave_mat", materia);
    urlencoded.append("calificacion", calificacion);
