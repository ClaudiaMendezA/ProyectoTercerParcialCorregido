...
/**
 * ********* SERVICIO PARA MODIFICAR ALUMNOS
 */
...
/**
 * ******** SERVICIO PARA MODIFICAR CALIFICACIONES
 */

/**
 * Acepta un id de alumno y retorna un arreglo de registros con los siguientes datos:
 * { nombreMateria, claveMateria, calificacion }
 */
$app->get('/alumno/{id}/calificaciones', function (Request $request, Response $response, array $args) {
    ...
    if(!empty($token)){
        // token existe
        $arrT = validaToken($token, $db, $conn); // valida token
        if($arrT['valido']){
            // token valido
            
            // id del alumno que viene de la URL: localhost/alumno/123/calificaciones
            $idAlumno = $args['id'];
            ...
            $sql = "SELECT 
                    ...
                    ";
            $respuesta = query($sql, $conn); // ejecuta consulta
            $arr['success'] = true;
            $arr['data'] = $respuesta;
        }else{
            $arr = array("errors"=> array("code"=>401, "detail" => "Token invalido"));
...
/**
 * Agrega una registro asociado a un alumno a la tabla evaluaciones
 * Acepta por URL el ID del alumno y por parametros, los datos a registrar
 */
$app->post('/alumno/{id}/calificaciones', function (Request $request, Response $response, array $args) {
    ...
    $data = $request->getParsedBody(); // obtener los datos del body, clave valor
    $arrJ = str_replace("'", "\"", $data);
    if(!isset($arrJ['token'])){ $arrJ['token'] = ""; }
    $token = $arrJ['token'];
    $arr = array();
...
/**
 * Edita una calificacion de un alumno
 */
$app->put('/alumno/{id}/calificaciones', function (Request $request, Response $response, array $args) {
    ...
    if(!empty($token)){
        ...
        if($arrT['valido']){
            ...
            // genera consulta sql
            $sql = "UPDATE {$db}.evaluaciones as ev
                    SET ev.calificacion = '{$data['calificacion']}' 
                    WHERE
                        ev.clave_alu = '{$idAlumno}' AND
                        ev.clave_mat = '{$data['clave_mat']}'
                    ";
...
/**
 * Elimina una calificacion de un alumno
 */
$app->delete('/alumno/{id}/calificaciones', function (Request $request, Response $response, array $args) {
    ...
    if(!empty($token)){
        ...
        if($arrT['valido']){
            ...
            // id del alumno que viene de la URL: localhost/alumno/123/calificaciones
            $idAlumno = $args['id'];
            $claveMateria = $arrJ['clave_mat'];

            // genera consulta sql
            $sql = "DELETE FROM  {$db}.evaluaciones
                    WHERE clave_alu = '{$idAlumno}' AND clave_mat = '{$claveMateria}'";
